...
const EXTRACTION_INSTRUCTIONS = [
  ...
  "Before reading numeric values, identify each numeral glyph's script: ASCII 0123456789, Arabic-Indic ٠١٢٣٤٥٦٧٨٩, or Eastern Arabic/Persian ۰۱۲۳۴۵۶۷۸۹.",
  "Use the predominant numeral script only as a consistency check. Mixed numeral scripts may appear on the same invoice; never reinterpret a clearly visible digit because the surrounding text is Arabic or English.",
  "Critical digit rule: Latin digit 0 is zero and must never be read as Arabic-Indic ٥ just because nearby text is Arabic. A clearly visible model token 10 must not become 15. Recheck every model, SKU, barcode, quantity, price, total, date, and invoice-number digit visually before returning it.",
  "Normalize confidently read digits in structured numeric fields and item names to ASCII 0-9 without changing their value. If a model, SKU, or barcode digit is unclear, do not guess; omit only the uncertain token if necessary and add a clear note to that item.",
  "For item names, preserve all visible item-type/category words such as battery, screen, protector, charger, بطارية, شاشة, حماية, شاحن, together with brand and model tokens. Do not shorten 'بطارية iPhone 15' to 'iPhone 15'.",
  "Read explicitly labeled header fields independently, especially supplier/vendor, invoice date, invoice number, and currency. For example, if the header visibly says المورد: محمد, supplierName must be محمد.",
  "For currency, return an ISO 4217 code such as EGP, SAR, USD, TRY, JOD, IQD, SYP, or AED only when the exact currency is clear. Do not return words such as جنيه or ريال; if the exact currency is ambiguous, return null.",
