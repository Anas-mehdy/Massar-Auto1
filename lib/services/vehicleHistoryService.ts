...
export type VehicleHistoryLaborLine = {
  id: string;
...
export type VehicleHistoryPartLine = {
  id: string;
  serviceOrderId: string;
  partName: string;
  quantity: number;
...
export type VehicleHistoryInspection = {
  id: string;
  serviceOrderId: string;
...
export type VehicleHistoryQuotation = {
  id: string;
  serviceOrderId: string;
  quoteNumber: string;
  revision: number;
...
export type VehicleHistoryApproval = {
  id: string;
  serviceOrderId: string;
...
export type VehicleHistoryPayment = {
  id: string;
  serviceOrderId: string;
  invoiceId: string;
  amount: number;
...
export type VehicleHistoryOrder = {
  ...
  laborLines: VehicleHistoryLaborLine[];
  partLines: VehicleHistoryPartLine[];
  inspections: VehicleHistoryInspection[];
  quotations: VehicleHistoryQuotation[];
  approvals: VehicleHistoryApproval[];
  payments: VehicleHistoryPayment[];
};
...
function groupByOrder<T extends { serviceOrderId: string }>(rows: T[]) {
  ...
  for (const row of rows) {
    ...
    else map.set(row.serviceOrderId, [row]);
  }
  return map;
}
...
export async function getVehicleLifetimeHistory(shopId: string, vehicleId: string): Promise<VehicleLifetimeHistory> {
  const [orders, laborLines, partLines, inspections, quotations, approvals, payments] = await Promise.all([
    prisma.$queryRaw<Array<Omit<VehicleHistoryOrder, "laborLines" | "partLines" | "inspections" | "quotations" | "approvals" | "payments">>>`
      SELECT
        ...
        inv."amountPaid"::double precision AS "invoiceAmountPaid",
        inv."balanceDue"::double precision AS "invoiceBalanceDue",
        inv."issuedAt" AS "invoiceIssuedAt"
    ...
    prisma.$queryRaw<VehicleHistoryLaborLine[]>`
      SELECT
        l."id", l."serviceOrderId", l."description", technician."name" AS "technicianName",
    ...
    prisma.$queryRaw<VehicleHistoryInspection[]>`
      SELECT
        si."id", si."serviceOrderId", si."inspectionType", si."status", si."summary",
        inspector."name" AS "inspectorName", si."inspectedAt", si."createdAt"
    ...
    prisma.$queryRaw<VehicleHistoryApproval[]>`
      SELECT
        ca."id", ca."serviceOrderId", ca."quotationId", ca."decision", ca."channel",
        ca."customerNameSnapshot", ca."note", ca."decidedAt",
        recorder."name" AS "recordedByName"
    ...
    prisma.$queryRaw<VehicleHistoryPayment[]>`
      SELECT
        p."id", inv."serviceOrderId", p."invoiceId",
        p."amount"::double precision AS "amount", p."method"::text AS "method",
  ...
  const laborByOrder = groupByOrder(laborLines);
  const partsByOrder = groupByOrder(partLines);
  const inspectionsByOrder = groupByOrder(inspections);
  const quotationsByOrder = groupByOrder(quotations);
  const approvalsByOrder = groupByOrder(approvals);
  const paymentsByOrder = groupByOrder(payments);

  const detailedOrders: VehicleHistoryOrder[] = orders.map((order) => ({
    ...order,
    laborLines: laborByOrder.get(order.id) ?? [],
    partLines: partsByOrder.get(order.id) ?? [],
    inspections: inspectionsByOrder.get(order.id) ?? [],
    quotations: quotationsByOrder.get(order.id) ?? [],
    approvals: approvalsByOrder.get(order.id) ?? [],
