...
export function InspectionForm({ serviceOrderId, disabled = false, lockedReason }: { serviceOrderId: string; disabled?: boolean; lockedReason?: string }) {
  ...
  async function submitInspection(formData: FormData) {
    ...
    setSaving(true);
    try {
      await createServiceInspectionAction(formData);
      setRows([newRow()]);
      setSummary("");
  ...
  return (
    <form action={submitInspection} className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="grid gap-2 text-xs font-black text-slate-700">
          <span>نوع الفحص</span>
          <select name="inspectionType" value={inspectionType} onChange={(e) => setInspectionType(e.target.value)} disabled={saving} className={inputClass}>
        ...
        <label className="grid gap-2 text-xs font-black text-slate-700">
          ...
          <input name="summary" value={summary} onChange={(e) => setSummary(e.target.value)} disabled={saving} className={inputClass} placeholder="مثال: يحتاج صيانة فرامل وتغيير زيت" />
        </label>
      </div>
        ...
        {rows.map((row, index) => (
          <div key={row.key} className="rounded-2xl border border-slate-200 bg-slate-50/60 p-3">
            <div className="mb-3 flex items-center justify-between gap-2">
              ...
              <button type="button" disabled={saving} onClick={() => removeRow(row.key)} className="rounded-lg p-2 text-slate-400 transition hover:bg-red-50 hover:text-red-600 disabled:cursor-not-allowed disabled:opacity-50" aria-label="حذف البند">
                <Trash2 className="h-4 w-4" />
              </button>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <input value={row.component} onChange={(e) => patchRow(row.key, { component: e.target.value })} required={index === 0} disabled={saving} className={inputClass} placeholder="المكوّن: فرامل، بطارية..." />
              <select value={row.result} onChange={(e) => patchRow(row.key, { result: e.target.value as InspectionResult })} disabled={saving} className={inputClass}>
              ...
              <input value={row.recommendedAction} onChange={(e) => patchRow(row.key, { recommendedAction: e.target.value })} disabled={saving} className={inputClass} placeholder="الإجراء المقترح" />
              <input value={row.estimatedCost} onChange={(e) => patchRow(row.key, { estimatedCost: e.target.value })} type="number" min="0" step="0.01" disabled={saving} className={inputClass} placeholder="تكلفة تقديرية" />
              <input value={row.notes} onChange={(e) => patchRow(row.key, { notes: e.target.value })} disabled={saving} className={`${inputClass} sm:col-span-2 xl:col-span-4`} placeholder="ملاحظات فنية على هذا البند" />
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:justify-between">
        <Button type="button" variant="outline" disabled={saving} onClick={() => setRows((current) => [...current, newRow()])} className="font-black">
          <Plus className="ml-1.5 h-4 w-4" />إضافة بند فحص
        </Button>
        <Button type="submit" disabled={saving || !rows.some((row) => row.component.trim())} className="font-black">
          <ClipboardCheck className="ml-1.5 h-4 w-4" />{saving ? "جارٍ حفظ الفحص..." : "حفظ الفحص"}
        </Button>
      </div>
    </form>
  );
