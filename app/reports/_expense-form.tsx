...
type FundingSource = "DRAWER" | "WALLET";
type WalletOption = { id: string; name: string; balance: number };
type CategoryOption = { value: string; label: string };

export function ExpenseForm({ categories, wallets, currency, todayInput, drawerBalance }: {
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button type="button" onClick={() => { setFundingSource("DRAWER"); setWalletId(""); }} className={`rounded-xl border p-3 text-right transition ${fundingSource === "DRAWER" ? "border-emerald-300 bg-emerald-50 text-emerald-800 dark:border-emerald-800 dark:bg-emerald-950/35 dark:text-emerald-200" : "border-slate-200 bg-white text-slate-600 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300"}`}>
            ...
            <p className="mt-1 text-[9px] font-semibold opacity-70">{drawerBalance == null ? "يُخصم المصروف مباشرة من رصيد الكاش." : `الرصيد الحالي: ${formatCurrency(drawerBalance, currency)}`}</p>
          </button>
          <button type="button" disabled={wallets.length === 0} onClick={() => setFundingSource("WALLET")} className={`rounded-xl border p-3 text-right transition disabled:cursor-not-allowed disabled:opacity-50 ${fundingSource === "WALLET" ? "border-indigo-300 bg-indigo-50 text-indigo-800 dark:border-indigo-800 dark:bg-indigo-950/35 dark:text-indigo-200" : "border-slate-200 bg-white text-slate-600 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300"}`}>
            ...
            <p className="mt-1 text-[9px] font-semibold opacity-70">{wallets.length ? "اختر المحفظة التي دفعت منها." : "لا توجد محافظ مفعلة."}</p>
          </button>
        </div>
      </div>
