...
export default async function DailySummaryPage() {
  ...
  const summary = await dailySummaryService.getDailySummary(auth.shop.id);
  const currency = auth.shop.currency || "SAR";
  const dayLabel = new Intl.DateTimeFormat("ar-EG", {
    weekday: "long",
    year: "numeric",
    month: "long",
    day: "numeric",
  ...
  return (
    <div className="space-y-7" dir="rtl">
      <PageHeader
        eyebrow="جرد يومي شامل"
        title="ملخص اليوم"
        description={`صورة مالية وتشغيلية كاملة ليوم ${dayLabel}. الأرقام اليومية محسوبة حسب توقيت المتجر.`}
        actions={(
      ...
      <section>
        <div className="mb-4"><h2 className="text-base font-black text-slate-900 dark:text-slate-100">نتيجة اليوم المالية</h2><p className="mt-1 text-xs font-bold text-slate-400">المبيعات والتكاليف والتحصيل والربح حتى هذه اللحظة.</p></div>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
          <SummaryMetric label="إجمالي مبيعات اليوم" value={formatCurrency(summary.totals.sales, currency)} helper="كل قنوات البيع المحققة" icon={ReceiptText} />
          <SummaryMetric label="المقبوض فعلياً" value={formatCurrency(summary.totals.collected, currency)} helper="كل الأموال المحصلة اليوم" icon={Banknote} />
          ...
          <SummaryMetric label="مصروفات اليوم" value={formatCurrency(summary.totals.expenses, currency)} helper="المصروفات المسجلة اليوم" icon={TrendingDown} />
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.6fr)]">
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950">
          <div className="border-b border-slate-100 px-5 py-4 dark:border-slate-800"><h2 className="text-sm font-black text-slate-900 dark:text-slate-100">المبيعات والربح حسب النشاط</h2><p className="mt-1 text-[11px] font-bold text-slate-400">تظهر التكلفة مع الإيراد في نفس اليوم. تكلفة الصيانة لا تدخل إلا عند وجود فاتورة غير ملغاة.</p></div>
        ...
        <div className="space-y-4">
          <CollectionPanel
            ...
            currency={currency}
          />
          <MoneyPanel title="السيولة والأرصدة الحالية" icon={WalletCards} rows={[
            ["إجمالي الأموال التشغيلية", summary.liquidity.total],
            ["الدرج النقدي", summary.liquidity.drawerBalance],
            ["المحافظ الإلكترونية", summary.liquidity.wallets.reduce((sum, wallet) => sum + wallet.balance, 0)],
            ["أرصدة مزودي الخدمات", summary.liquidity.providers.reduce((sum, provider) => sum + provider.balance, 0)],
          ]} currency={currency} />
        </div>
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          <div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-50 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300"><Boxes className="h-5 w-5" /></span><div><h2 className="text-sm font-black text-slate-900 dark:text-slate-100">المخزون والتوالف</h2><p className="mt-1 text-[10px] font-bold text-slate-400">قيمة المخزون الحالية والتالف المسجل اليوم.</p></div></div>
          <div className="mt-4 grid gap-3 sm:grid-cols-2"><MiniValue label="قيمة المخزون بسعر التكلفة" value={formatCurrency(summary.inventory.valueAtCost, currency)} /><MiniValue label="قيمة التوالف اليوم" value={formatCurrency(summary.inventory.damageValue, currency)} helper={`${summary.inventory.damageCount} حركة تالف`} /></div>
          {summary.inventory.damageItems.length ? <div className="mt-4 border-t border-slate-100 pt-4 dark:border-slate-800"><p className="mb-2 text-[10px] font-black text-slate-500">المنتجات التي سجل عليها تالف اليوم</p><div className="flex flex-wrap gap-2">{summary.inventory.damageItems.map((item) => <Button key={item.inventoryItemId} asChild size="sm" variant="outline" className="rounded-lg border-rose-200 text-rose-700 dark:border-rose-900 dark:text-rose-300"><Link href={`/inventory/${item.inventoryItemId}#inventory-movements`}><PackageX className="ml-1 h-3.5 w-3.5" />{item.name} ({item.quantity})</Link></Button>)}</div></div> : <p className="mt-4 text-xs font-bold text-slate-400">لا توجد توالف مسجلة اليوم.</p>}
        </div>
      </section>

      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        <div className="mb-4"><h2 className="text-sm font-black text-slate-900 dark:text-slate-100">الذمم والالتزامات الحالية</h2><p className="mt-1 text-[10px] font-bold text-slate-400">هذه أرصدة حالية حتى الآن وليست محصورة بما حدث اليوم.</p></div>
      ...
      {summary.electronicCategories.length > 0 ? (
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          <div className="mb-4"><h2 className="text-sm font-black text-slate-900 dark:text-slate-100">تفصيل الخدمات الإلكترونية اليوم</h2><p className="mt-1 text-[10px] font-bold text-slate-400">تفصيل حسب التصنيفات الفعلية التي يستخدمها المتجر.</p></div>
...
function CollectionPanel({ sources, collected, outstanding, transferProfit, currency }: {
  ...
  return <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
    <div className="flex items-center gap-2"><CreditCard className="h-5 w-5 text-primary" /><div><h3 className="text-sm font-black text-slate-900 dark:text-slate-100">أين دخل المقبوض اليوم؟</h3><p className="mt-1 text-[9px] font-bold text-slate-400">تفصيل ديناميكي حسب الدرج والمحافظ ومصادر التحصيل المسجلة.</p></div></div>
    <div className="mt-4 rounded-xl bg-slate-50 px-3 py-3 dark:bg-slate-900"><div className="flex items-center justify-between gap-3"><span className="text-[11px] font-black text-slate-600 dark:text-slate-300">إجمالي المقبوض فعلياً اليوم</span><span className="font-numeric text-base font-black text-slate-950 dark:text-white">{formatCurrency(collected, currency)}</span></div></div>
    ...
    }) : <p className="rounded-xl border border-dashed border-slate-200 p-3 text-center text-[10px] font-bold text-slate-400 dark:border-slate-800">لا توجد مقبوضات مسجلة اليوم.</p>}</div>
    <div className="mt-4 grid gap-2 border-t border-slate-100 pt-4 dark:border-slate-800">
      <div className="flex items-center justify-between gap-3"><span className="text-[10px] font-bold text-slate-500">المتبقي من أعمال اليوم</span><span className="font-numeric text-sm font-black text-amber-700 dark:text-amber-300">{formatCurrency(outstanding, currency)}</span></div>
    ...
    <p className="mt-3 text-[9px] font-bold leading-5 text-slate-400">المقبوض اليوم هو حركة الأموال التي دخلت اليوم، وقد يشمل تحصيل فاتورة أو دين أو قسط أقدم. لذلك لا نخلطه مع «إجمالي مبيعات اليوم» عند وجود تحصيلات قديمة أو مبيعات آجلة.</p>
  </div>;
}
