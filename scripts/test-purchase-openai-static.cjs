...
assert.match(provider, /schemaVersion:\s*\{\s*type:\s*"integer",\s*enum:/);
assert.doesNotMatch(provider, /schemaVersion:\s*\{[^}]*\bconst:/);
assert.match(provider, /OPENAI_INVALID_REQUEST/);
...
assert.match(service, /const quotaCharged = providerContacted/);
